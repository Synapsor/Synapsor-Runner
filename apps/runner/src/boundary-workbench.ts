  ...
  <style>
    ...
    *{box-sizing:border-box;letter-spacing:0}
    ...
    .ask-boundary-examples{display:flex;gap:8px;flex-wrap:wrap;margin-top:16px}.ask-boundary-examples .question{width:auto;min-height:38px;padding:8px 11px;font-size:13px}
	    ...
	    .boundary-catalog-path{border-left:2px solid #75e3b7;background:#0b1511}.boundary-catalog-path.unproven{border-left-color:#f0aa68}.boundary-catalog-path .boundary-catalog-edge{border-left:0;background:transparent}.boundary-catalog-question{margin:0;padding:0 12px 11px;color:#cbd6d0;font-size:12px}.boundary-catalog-question strong{color:#75e3b7}.boundary-catalog-questions{margin:12px 0;padding:12px 14px;border-left:2px solid #75e3b7;background:#0b1511}.boundary-catalog-questions strong{color:#75e3b7}.boundary-catalog-questions ul{margin:8px 0 0;padding-left:20px}.boundary-catalog-questions li+li{margin-top:5px}.boundary-catalog-capabilities{color:#aab8b1!important}.boundary-catalog-legend{margin:10px 0 12px;color:#839189;font-size:12px}.boundary-field-matrix-wrap{margin-top:10px;overflow-x:auto}.boundary-field-matrix{width:100%;min-width:620px;border-collapse:collapse;font-size:11px}.boundary-field-matrix th,.boundary-field-matrix td{padding:6px 7px;border-bottom:1px solid #26372f;text-align:center;white-space:nowrap}.boundary-field-matrix th:first-child{width:32%;text-align:left}.boundary-field-matrix thead th{color:#839189;font-size:10px}.boundary-field-matrix tbody th{font-weight:500;white-space:normal;overflow-wrap:anywhere}.boundary-field-matrix tbody th small{margin:2px 0 0;color:#839189}.boundary-field-matrix abbr{text-decoration:none;cursor:help}.boundary-field-exact{margin:8px 0 0;padding-top:8px}.boundary-field-exact ul{display:grid;gap:4px;margin:8px 0 0;padding-left:20px;color:#839189}.boundary-field-exact li code{margin-right:8px;color:#aab8b1}.boundary-catalog-restrictions{margin-top:9px!important;color:#e7bd75!important}.boundary-relationship-summary{display:grid;grid-template-columns:62px minmax(0,1fr);gap:5px 12px;padding:11px 13px}.boundary-relationship-summary>strong{grid-row:1/5;color:#75e3b7}.boundary-relationship-summary>code{overflow-wrap:anywhere}.boundary-relationship-summary>small,.boundary-relationship-summary>span{color:#839189}.boundary-relationship-summary>details summary{cursor:pointer;color:#839189}.boundary-relationship-summary>details code{display:block;margin-top:5px;overflow-wrap:anywhere;color:#aab8b1}
...
<body>
  ...
  <script>
    ...
    function boundaryMapMark(value,label){
      return '<td aria-label="'+esc(label+': '+(value?'reviewed':'unavailable'))+'" title="'+esc(label+': '+(value?'reviewed':'unavailable'))+'">'+(value?'Y':'-')+'</td>';
    }
    ...
    function renderBoundaryFieldMatrix(table){
      ...
      const rows=fields.map(field=>{
        ...
        return '<tr><th scope="row"><code>'+esc(field.name)+'</code><small>'+esc(field.data_type||'reviewed')+'</small></th>'
          +boundaryMapMark(operations.return_value!==false,'Return')
          +boundaryMapMark(filterOperators.length>0,'Filter')
          +boundaryMapMark(operations.sort===true,'Sort')
          +boundaryMapMark(group,'Group or band')
          +boundaryMapMark(measure,'Numeric measure')
          +boundaryMapMark(operations.presence===true,'Missing-data measure')
          +boundaryMapMark(distinct,'Distinct count')
          +boundaryMapMark(time,'Time bucket')
          +'</tr>';
      }).join('');
      return '<div class="boundary-field-matrix-wrap"><table class="boundary-field-matrix"><thead><tr><th>Field</th><th><abbr title="Return">RET</abbr></th><th><abbr title="Filter">FLT</abbr></th><th><abbr title="Sort">SRT</abbr></th><th><abbr title="Group or numeric band">GRP</abbr></th><th><abbr title="Numeric measure">MEA</abbr></th><th><abbr title="Missing-data measure">PRE</abbr></th><th><abbr title="Distinct count">DST</abbr></th><th><abbr title="Time bucket">TIM</abbr></th></tr></thead><tbody>'+rows+'</tbody></table></div>'
    ...
    function renderBoundaryRelationshipMap(model,diagrams){
      ...
      const sections=boundaries.map(boundary=>{
        ...
        const detail=large?'':'<details><summary>Reviewed fields and relationships</summary><p class="boundary-catalog-legend">Y = reviewed; - = unavailable. RET return · FLT filter · SRT sort · GRP group/band · MEA numeric measure · PRE missing-data measure · DST distinct count · TIM time bucket.</p><div class="boundary-catalog-nodes">'+nodes+'</div>'+(edges?'<div class="boundary-catalog-edges">'+edges+'</div>':'')+'</details>';
