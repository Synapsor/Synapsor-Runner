	      ...
	      byId("resource-search").placeholder="Search "+collectionLabel;
		    ...
		    function renderBoundaryOverview(){
		      ...
		      const rankedCurrent=candidate.budgets.max_ranked_groups??rankedMinimum;
			      ...
			      const cohortSettings='<details class="boundary-options"><summary>Privacy for all tables'+(cohortValues.length===1?' · minimum '+esc(cohortCurrent):' · mixed thresholds')+'</summary><div class="boundary-name-editor"><label>Minimum cohort for every included table<select id="boundary-cohort-all"><option value="5" '+(cohortCurrent===5?'selected':'')+'>5 — default</option><option value="4" '+(cohortCurrent===4?'selected':'')+'>4 — owner override</option><option value="3" '+(cohortCurrent===3?'selected':'')+'>3 — owner override</option><option value="2" '+(cohortCurrent===2?'selected':'')+'>2 — high disclosure risk</option><option value="1" '+(cohortCurrent===1?'selected':'')+'>No suppression — effective minimum 1</option></select></label><label>Human reviewer<input id="boundary-cohort-actor" type="text" maxlength="128" value="'+esc(byId("actor").value.trim())+'"></label><label>Why is this appropriate?<textarea id="boundary-cohort-reason" maxlength="500" rows="2" placeholder="Explain why this threshold is appropriate for every table in this boundary."></textarea></label><button id="save-boundary-cohort" class="secondary" type="button" '+(candidate.pack.resources.length?'':'disabled')+'>Apply to all '+esc(candidate.pack.resources.length)+' table'+(candidate.pack.resources.length===1?'':'s')+'</button><span id="boundary-cohort-status" class="status-message" aria-live="polite"></span><small>Runner hides smaller aggregate groups by default. No suppression permits groups of one, which can identify individuals. This owner decision is saved atomically for every included table, remains disabled, and requires separate activation.</small></div></details>';
			      panel.innerHTML=
			      ...
			      if(cohortAllSave&&!cohortAllSave.disabled)cohortAllSave.onclick=async()=>{
			        ...
			        try{
			          if(!Number.isSafeInteger(value)||value<1||value>5||!actor||!reason){
			            throw new Error("Choose a threshold and enter the human reviewer identity and reason.");
			          }
			          status.className="status-message";
        ...
        status.textContent=completedOperation
          ...
          :visibleCalls.length
            ?refusedSourceExecuted
              ?"The read-only result was discarded. Choose another question or review the table privacy threshold."
    ...
    function renderAskToolResult(call){
      ...
      if(call.tool==="app.explore_data"&&plan&&typeof plan==="object"){
        ...
        return '<section class="ask-tool-trace"><p>'+esc(planSentence(plan,boundaryName))+'</p>'
          ...
          +verifiedData
		          +(privacyGuidance?'<p><strong>'+esc(suppressed)+' additional group'+(suppressed===1?" was":"s were")+' withheld because '+(suppressed===1?"it was":"they were")+' below the reviewed minimum cohort'+(minimumCohort?' of '+esc(minimumCohort):'')+'.</strong></p>'+(privacyGuidance.shape?'<p>'+esc(privacyGuidance.shape)+'</p>':'')+'<p>'+esc(privacyGuidance.path)+'</p><button class="quiet" data-review-privacy-resource="'+esc(plan.resource)+'" type="button">Review privacy for '+esc(plan.resource)+'</button>':'')
	    ...
	    function suppressionReviewGuidance(plan,boundaryName,minimumCohort){
	      ...
	      const shape=field&&/(^id$|_id$|(^|_)name$)/i.test(field)
	        ?"This question groups records into one row per "+fieldLabel(resource,field).toLowerCase()+"; any entity with fewer than "+(minimumCohort||"the reviewed minimum")+" records is withheld. Try a coarser reviewed grouping, or explicitly review the staging threshold."
	        :"";
	      return {
	        shape,
	        path:"To change it: boundary "+resolvedBoundary+" -> table "+plan.resource+" -> Privacy -> choose threshold -> Review and activate. Until activation, Ask uses the previous threshold. No suppression uses an effective minimum of 1."
	      };
	    }
