...
<body>
  ...
  <script>
    ...
    function renderDatabaseCompatibility(value){const panel=document.getElementById("database-compatibility");if(!value){panel.hidden=true;return}const limited=value.tier==="compatible_limited";panel.hidden=false;panel.className="notice compatibility"+(limited?"":" full");panel.innerHTML='<strong>Database compatibility: '+esc(value.detected_version)+'</strong><br>'+(limited?'Supported limited grammar. Text grouping and filtering require a bounded native ENUM; automatic numeric bands are unavailable.':'Full reviewed grammar is available for this supported server line.')}
