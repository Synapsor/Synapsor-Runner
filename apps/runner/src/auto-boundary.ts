...
function buildExplorationBoundaryDraft(
  ...
  if (options.deploymentProfile === "production" && !options.httpClaims) {
    throw new Error(options.organizationScope
  ...
  if (options.deploymentProfile === "production"
    && !options.organizationScope
    && !options.httpClaims?.tenantClaim) {
    throw new Error("Multi-tenant production Explore requires an explicit tenant HTTP claim name.");
  }
  if (options.deploymentProfile === "production"
    && options.organizationScope
    && options.httpClaims?.tenantClaim) {
    throw new Error("Single-organization production Explore uses its reviewed fixed organization id and must not configure a tenant HTTP claim.");
  }
  if (options.deploymentProfile !== "production" && options.httpClaims) {
  ...
  const draft: ExplorationBoundaryDraft = {
    ...
    trusted_context: options.deploymentProfile === "production"
      ? {
        ...
        ...(options.httpClaims!.tenantClaim ? { tenant_claim: options.httpClaims!.tenantClaim } : {}),
